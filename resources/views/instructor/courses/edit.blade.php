    ...
    <x-slot name="header">
        <div class="flex flex-col md:flex-row md:justify-between items-center">
            <h2 class="font-semibold text-xl text-gray-800 dark:text-gray-400 leading-tight">
                Actualizar Curso : <span class="text-gray-200">{{ $course->title }}</span>
            </h2>
    ...
    <x-contenido class="py-12" width="4xl">
        <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg p-6">
            <form action="{{ route('instructor.courses.update', $course->id) }}" method="POST" enctype="multipart/form-data">
                @csrf
                @method('PUT')
                <p class="text-gray-700 dark:text-white text-md font-semibold py-2">Completa la información para actualizar el curso</p>
                ...
                <div class="flex flex-col mt-4">
                    <div>
                        ...
                        <x-input for="title" placeholder="Nombre del Curso" name="title" class="mb-2 w-full"
                            value="{{ old('title', $course->title) }}" id="title" oninput="generateSlug(this.value)"  />
                    </div>
                </div>
                <div class="flex flex-col mt-4">
                    <div>
